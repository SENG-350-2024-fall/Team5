...
export const loadJsonIntoObject = (filePath: string): Array<TRIAGE_APPLICATION> | Array<SYMPTOM> | Array<HEALTH_CARE_WORKER> | Array<OFFICE> | Array<PATIENT> | null => {
  try {
    const jsonData = fs.readFileSync(filePath, 'utf8');  // Synchronously read file content
    const data = JSON.parse(jsonData) as Array<TRIAGE_APPLICATION> | Array<SYMPTOM> | Array<HEALTH_CARE_WORKER> | Array<OFFICE> | Array<PATIENT>; // Parse JSON content into an object
    return data;
  } catch (error) {
